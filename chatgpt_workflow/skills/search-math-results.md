...
8. Archive papers with download_paper (arXiv) or save_file (other sources); read extracted text with read_file, following all pages. Local users can import PDFs with --import-file. Inspect extraction quality before relying on it.
9. Record not only what the theorem says, but also what its proof suggests for the current problem.
10. If the theorem search returns no useful information, switch to ChatGPT web search when available.
...
Save this structured summary as JSON text in a record_artifacts record (channel=branch_states):
...
If neither theorem search nor web search yields useful information, save a branch_states artifact with:
