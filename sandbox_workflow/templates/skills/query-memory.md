...
3. Run `python3 research.py search --query "TEXT" --channel CHANNEL` once per selected channel. Read local records directly if a lexical query misses relevant material; there are no relevance scores promised by this CLI.
...
<!-- Adapted from agents/generation/.agents/skills/query-memory/SKILL.md; preserve the repository license and NOTICE. -->
