...
Use only when `blueprint.md` contains a full candidate proof of the original problem, not isolated lemmas, a partial branch, or exploratory notes. Read the entire draft, the original statement, and previous reports. Check that the main theorem reproduces the complete input statement and every required argument is supplied.
